...
```json
{
  "ParticipantSC": {
    ...
    "identifierValue": "60fc8713...",
    "personName": {
      "firstName": "James",
      "lastName": "Cameron"
    },
    "Location": "me-nexus:d1864144...",
...
**Key Changes:**
1. ✅ No schemaVersion in nested object
2. ✅ identifierScope/Value moved into identifier array
3. ✅ firstName/lastName → givenName/familyName
4. ✅ Location is object reference, not string
5. ✅ No double-nested identifier
...
1. Fix RDF location string literals (5 minutes)
2. Implement property name mapping (2 hours)
3. Fix Location reference handling (1 hour)

**Week 2 - Quality Fixes:**
4. Clean nested object metadata (1 hour)
5. Fix identifier structure (1 hour)
6. Handle blank nodes properly (2 hours)
7. Populate Infrastructure structural properties (30 minutes)

**Week 3 - Testing:**
8. Build validation tests
9. Compare against official schema examples
10. Round-trip testing (JSON → RDF → JSON)

---

## 🚨 **CRITICAL REMINDER**

**Your JSON does NOT currently pass OMC validation.**

The transformation layer you have now:
- Carries over RDF bugs (location string)
- Doesn't map property names
- Exports blank node IDs as strings
- Creates malformed identifier structures

**Bottom line:** You need to fix BOTH the RDF source AND the JSON serializer to get valid OMC-compliant JSON.

Start with the RDF fixes (15 minutes), then tackle the property mapping layer (2-3 hours).
