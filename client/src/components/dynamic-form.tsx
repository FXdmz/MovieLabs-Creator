...
export function SchemaField({ fieldKey, schema, value, onChange, path = "", level = 0, rootSchema, entityType }: SchemaFieldProps & { rootSchema?: any }) {
  ...
  if (schemaType === 'object' || schema.properties) {
    ...
    const isCustomDataObject = !hasDefinedProperties && level > 0;
    
    // Custom data object - allow arbitrary key-value pairs
    if (isCustomDataObject) {
      const [newKey, setNewKey] = useState('');
      const objValue = (typeof value === 'object' && value !== null) ? value : {};
      const entries = Object.entries(objValue);
      
      const addCustomProperty = () => {
        if (newKey.trim() && !objValue[newKey.trim()]) {
          onChange({ ...objValue, [newKey.trim()]: '' });
          setNewKey('');
        }
      };
          ...
          <Collapsible open={isOpen} onOpenChange={setIsOpen}>
            <CardHeader className="py-3 px-4 flex flex-row items-center justify-between space-y-0 cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setIsOpen(!isOpen)}>
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="font-normal text-[10px]">{entries.length} items</Badge>
              </div>
            </CardHeader>
            <CollapsibleContent>
              <CardContent className="pt-0 pb-4 px-4 space-y-3">
                ...
                <div className="space-y-2">
                    ...
                    <div key={key} className="flex items-start gap-2 group">
                      <div className="flex-1 space-y-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => onChange({ ...objValue, [key]: e.target.value })}
                      ...
                      <Button 
                        variant="ghost" 
                        size="icon" 
                        className="h-8 w-8 mt-5 text-destructive opacity-0 group-hover:opacity-100 transition-opacity hover:bg-destructive/10"
                        onClick={() => {
                  ...
                  {entries.length === 0 && (
                    <div className="text-xs text-muted-foreground italic py-2">No custom properties</div>
                  )}
                </div>
              </CardContent>
            </CollapsibleContent>
